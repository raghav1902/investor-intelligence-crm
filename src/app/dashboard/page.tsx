            ...
            <table className="w-full text-left text-xs whitespace-nowrap min-w-[1000px]">
              <thead className="border-b border-hairline bg-surface-200 tracking-tight text-content-secondary font-medium transition-colors">
                <tr>
                  ...
                  <th className="px-4 py-3.5 cursor-pointer hover:text-content-primary transition" onClick={() => handleSort('emailDomain')}>Email Domain {sortBy === 'emailDomain' ? (sortOrder === 'asc' ? '↑' : '↓') : ''}</th>
                  {Array.from(customFieldKeys).map(key => (
                    <th key={key} className="px-4 py-3.5 cursor-pointer hover:text-content-primary transition text-emerald-400" onClick={() => handleSort(`customFields.${key}`)}>
                      {key} {sortBy === `customFields.${key}` ? (sortOrder === 'asc' ? '↑' : '↓') : ''}
                    </th>
                  ))}
                  <th className="px-4 py-3.5 cursor-pointer hover:text-content-primary transition" onClick={() => handleSort('status')}>Status {sortBy === 'status' ? (sortOrder === 'asc' ? '↑' : '↓') : ''}</th>
